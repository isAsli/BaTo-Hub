...
if ! need_cmds curl ca-certificates openssl unzip rsync python3 jq; then
...
if ! apt-get install -y -qq curl ca-certificates openssl unzip rsync python3 jq; then
...
find "$BASE/bin" "$BASE/core" "$BASE/lib" "$BASE/security" \
  "$BASE/panels" "$BASE/panels"/*/ssl "$BASE/panels"/*/templates \
