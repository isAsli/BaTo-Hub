...
ssl_menu() {
  while :; do
    clear
    banner "Rebecca / SSL"
    printf '%s\n' '1) Obtain and install SSL' '2) Renew SSL' '3) SSL status' '4) Remove SSL settings' '0) Back'
...
ssl_install() {
  ...
  certdir="$REBECCA_DIR/certs/$domain"
  install -d -m 750 "$certdir"
  chown -R root:root "$REBECCA_DIR/certs"
...
HOOK
  ...
  set_env REBECCA_CERT_BASE "$REBECCA_DIR/certs"
